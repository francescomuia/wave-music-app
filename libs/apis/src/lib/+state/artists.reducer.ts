...
export const artistsAdapter: EntityAdapter<ArtistsEntity> =
  createEntityAdapter<ArtistsEntity>();
...
const artistsReducer = createReducer(
  ...
  on(ArtistsActions.loadArtistsSuccess, (state, { artists }) => {
    let newArtists: ArtistsEntity[] = [];
    for (let key in state.entities) {
      newArtists.push(<ArtistsEntity>state.entities[key]);
    }
    newArtists = newArtists.concat(artists);
    return artistsAdapter.setAll(newArtists, { ...state, loaded: true });
  }),
